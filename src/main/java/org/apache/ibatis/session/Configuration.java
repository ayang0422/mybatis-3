...
public class Configuration {
  ...
  /**
   * 允许在嵌套语句中使用分页。如果允许，则设置为false。默认false
   */
  ...
  /**
   * 指定日志的具体实现，未指定时将自动查找。一般建议指定slf4j或者log4j
   */
  protected Class<? extends Log> logImpl;
  /**
   * VFS=virtual file system
   * 指定VFS的实现，VFS时mybatis提供的用于访问AS内部的一个简便的接口
   */
  ...
  /**
   * 对象工厂，对于所有的ResultMap类型都需要依赖对象工厂来实例化
   */
  ...
  /**
   * 延迟加载的全局开关，当开启时，所有关联对象都会延迟加载，特定关联关系可以通过fetchType属性来覆盖该项的开关状态。
   */
  ...
  // 用于在执行sql语句的出入参映射以及mybatis-config文件里的各种配置
  // 比如<transactionManager type="JDBC"/><dataSource typeO="POOLED">时使用简写
  ...
  public Executor newExecutor(Transaction transaction, ExecutorType executorType) {
    ...
    // 默认 cacheEnabled = true
    // CachingExecutor使用的是装饰者模式，它内部持有一个Executor对象，
    // 可能是SimpleExecutor/ BatchExecutor / ReuseExecutor
    if (cacheEnabled) {
